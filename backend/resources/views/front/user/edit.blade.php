        ...
        <div id="personal_information">
            ...
            <div>
                ...
                @foreach ($configs['genders'] as $gender => $description)
                    <input type="radio" name="gender" id="{{ $gender }}" value="{{ $gender }}" {{ old('gender', $user->gender->value) === $gender ? 'checked' : '' }} disabled>
            ...
            <div>
                ...
                <span id="birthday">
                    <input type="text" name="birth_year" value="{{ old('birth_year', (int) $user->birth_year) }}">年
                    <input type="text" name="birth_month" value="{{ old('birth_month', (int) $user->birth_month) }}">月
                    <input type="text" name="birth_date" value="{{ old('birth_date', (int) $user->birth_date) }}">日
                </span>
                @error('birth_year')
                    <span>{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="height" value="{{ old('height', $user->height) }}">cm
                @error('height')
                    <span>{{ $message }}</span>
                @enderror
            </div>
            <div>
                ...
                <input type="text" name="weight" value="{{ old('weight', $user->weight) }}">kg
                @error('weight')
                    <span>{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="prefecture">
                    @foreach ($configs['prefectures'] as $prefecture => $description)
                        <option value="{{ $prefecture }}" {{ old('prefecture', $user->prefecture->value) === $prefecture ? 'selected' : '' }}>{{ $description }}</option>
                    @endforeach
                </select>
                @error('prefecture')
                    <span>{{ $message }}</span>
            ...
            <div>
                ...
                <input type="text" name="address_building" value="{{ old('address_building', $user->address_building) }}">
            ...
            <div>
                ...
                <select name="plan_id">
                    @foreach ($configs['plans'] as $plan)
                        <option value="{{ $plan->id }}" {{ old('plan_id', $user->plan->id) == $plan->id ? 'selected' : '' }}>{{ $plan->description }}</option>
                    @endforeach
                </select>
                @error('plan_id')
                    <span>{{ $message }}</span>
